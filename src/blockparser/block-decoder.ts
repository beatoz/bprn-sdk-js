...
export class BlockDecoder {
	...
	private static decodePayload(payload: fabprotos.common.IPayload): DecodedPayload {
		...
		if (payload.header) {
			if (payload.header.channel_header) {
				decodedPayload.header!.channel_header = fabprotos.common.ChannelHeader.decode(
					payload.header.channel_header as Uint8Array
				);
			}

			if (payload.header.signature_header) {
				decodedPayload.header!.signature_header = fabprotos.common.SignatureHeader.decode(
					payload.header.signature_header as Uint8Array
				);
			}
		}
	...
	private static decodeTransactionAction(action: fabprotos.protos.ITransactionAction): DecodedTransactionAction {
		...
		if (action.payload) {
			const chaincodeActionPayload = fabprotos.protos.ChaincodeActionPayload.decode(
				action.payload as Uint8Array
			);
	...
	private static decodeChaincodeActionPayload(
		payload: fabprotos.protos.IChaincodeActionPayload
	): DecodedChaincodeActionPayload {
		const decoded: DecodedChaincodeActionPayload = {};

		if (payload.chaincode_proposal_payload) {
			const proposalPayload = fabprotos.protos.ChaincodeProposalPayload.decode(
				payload.chaincode_proposal_payload as Uint8Array
			);
	...
	private static decodeChaincodeProposalPayload(
		payload: fabprotos.protos.IChaincodeProposalPayload
	): DecodedChaincodeProposalPayload {
		...
		if (payload.input) {
			const invocationSpec = fabprotos.protos.ChaincodeInvocationSpec.decode(
				payload.input as Uint8Array
			);
	...
	private static decodeChaincodeInvocationSpec(
		spec: fabprotos.protos.IChaincodeInvocationSpec
	): DecodedChaincodeInvocationSpec {
	...
	private static decodeChaincodeSpec(
		spec: fabprotos.protos.IChaincodeSpec
	): DecodedChaincodeSpec {
	...
	private static decodeChaincodeEndorsedAction(
		action: fabprotos.protos.IChaincodeEndorsedAction
	): DecodedChaincodeEndorsedAction {
		...
		if (action.proposal_response_payload) {
			const responsePayload = fabprotos.protos.ProposalResponsePayload.decode(
				action.proposal_response_payload as Uint8Array
			);
	...
	private static decodeProposalResponsePayload(
		payload: fabprotos.protos.IProposalResponsePayload
	): DecodedProposalResponsePayload {
		...
		if (payload.extension) {
			const chaincodeAction = fabprotos.protos.ChaincodeAction.decode(
				payload.extension as Uint8Array
			);
	...
	private static decodeChaincodeAction(
		action: fabprotos.protos.IChaincodeAction
	): DecodedChaincodeAction {
		...
		if (action.results) {
			const txReadWriteSet = fabprotos.rwset.TxReadWriteSet.decode(
				action.results as Uint8Array
			);
		...
		// Decode events
		if (action.events) {
			decoded.events = fabprotos.protos.ChaincodeEvent.decode(
				action.events as Uint8Array
			);
		}
	...
	private static decodeTxReadWriteSet(
		rwset: fabprotos.rwset.ITxReadWriteSet
	): DecodedTxReadWriteSet {
		...
		if (rwset.ns_rwset) {
			for (const nsRwset of rwset.ns_rwset) {
				...
				if (nsRwset.rwset) {
					decodedNsRwset.rwset = fabprotos.kvrwset.KVRWSet.decode(
						nsRwset.rwset as Uint8Array
					);
				}
				...
				if (nsRwset.collection_hashed_rwset) {
					...
					for (const collHashedRwset of nsRwset.collection_hashed_rwset) {
						...
						if (collHashedRwset.hashed_rwset) {
							decoded.hashed_rwset = fabprotos.kvrwset.HashedRWSet.decode(
								collHashedRwset.hashed_rwset as Uint8Array
							);
						}
