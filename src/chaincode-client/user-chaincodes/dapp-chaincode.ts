...
export class DappChaincode {
	...
	async init(cliInvoker: CliChaincodeInvoker, ownerAccount: ChaincodeSigner) {
		...
		const signature = await signChaincodeRequest(ownerAccount, this.chaincode.createSignatureRequest("", methodName, []))
	...
	async invoke(signer: ChaincodeSigner, functionName: string, args: string[]): Promise<any> {
